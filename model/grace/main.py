...
def run(config: DictConfig):
    ...
    if config.wandb:
        wandb.init(
            project=config.wandb_project_name, config=config, mode=config.wandb_mode
        )
        if not config.wandb_run_name:
            ...
            try:
                wandb.run.name = (
                    f"{str(config.editor._name)}_{os.getenv('SLURM_JOBID')}"
                )
            except:
                pass
        wandb.run.save()
    ...
    if config.experiment.task == "qa":
        ...
        upstream_loader = DataLoader(
            upstream, batch_size=config.batch_size, shuffle=False
        )
        holdout = 0
        ...
        upstream = WebText10k()
        edit_loader = DataLoader(edits, batch_size=1, shuffle=False)
        accurate_loader = DataLoader(
            accurate_dataset, batch_size=config.batch_size, shuffle=False
        )
        upstream_loader = DataLoader(
            upstream, batch_size=config.batch_size, shuffle=False
        )
        ...
        is_error = (
            lambda *args: True
        )  # For language modeling, we've precomputed which inputs we count as "errors". In principle, this could be thresholded PPL or something.
    ...
    with torch.no_grad():
        original_edits = torch.tensor(
            [
                metric(editor, tokenize(e, editor.tokenizer, config["device"]))
                for e in iter(edit_loader)
            ]
        ).nanmean()  # Log first PPL before edits
        ...
        if config.experiment.task == "hallucination":
            ARR = torch.tensor(
                [
                    metric(
                        editor,
                        tokenize(e, editor.tokenizer, config["device"], test=True),
                    )
                    for e in iter(accurate_loader)
                ]
            ).nanmean()  # Log first PPL before edits
        ...
        TRR = torch.tensor(
            [
                metric(
                    editor, tokenize(e, editor.tokenizer, config["device"], test=True)
                )
                for e in iter(upstream_loader)
            ]
        )
    ...
    for i, batch in tqdm(enumerate(edit_loader)):
        ...
        if is_error(editor, tokens) & (n_edits <= config.max_n_edits):
            ...
            if "retrain" in config.editor._name and (
                i > 0 and n_edits % config.editor.retrain_frequency == 0
            ):
                retrain_start = time()
            ...
            with torch.no_grad():
                ...
                if (i > 0 and n_edits % config.metric_period == 0) or (
                    i == len(edit_loader) - 1
                ):  # Compute historical metrics every k edits to save time
                    if config.experiment.task == "hallucination":
                        ARR = torch.tensor(
                            [
                                metric(
                                    editor,
                                    tokenize(e, editor.tokenizer, config["device"]),
                                )
                                for e in iter(accurate_loader)
                            ]
                        ).nanmean()
                        ...
                        holdout = torch.tensor(
                            [
                                ...
                                for e in iter(edit_holdout_loader)
                            ]
                        ).nanmean()
                    else:
                        pass
                    ...
                    ERR = torch.tensor(
                        [metric(editor, tokens) for tokens in batch_history]
                    ).nanmean()
                    TRR = torch.tensor(
                        [
                            metric(
                                editor,
                                tokenize(
                                    e, editor.tokenizer, config["device"], test=True
                                ),
                            )
                            for e in iter(upstream_loader)
                        ]
            ...
            if hasattr(editor, "log_dict"):
                log_dict.update(
                    editor.log_dict
                )  # Add any logged values inside our editor to log_dict
