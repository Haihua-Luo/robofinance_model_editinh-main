...
class NQ(Dataset):
    def __init__(self, path="./grace/data/nq_train.json"):
        ...
        for x, y in zip(questions[:1000], answers[:1000]):
            self.data.append({"text": x, "labels": y})
...
class zsRE(Dataset):
    def __init__(
        self,
        path="./grace/data/structured_zeroshot-dev-new_annotated_final.jsonl",
        split="edit",
    ):
    ...
    def load_zsre(self, data_path):
        ...
        with jsonlines.open(data_path) as f:
            for d in f:
                ...
                if (
                    len(ex["filtered_rephrases"]) >= 10
                ):  # Only use samples for which there are 10 rephrasings
                    for rephrase in ex["filtered_rephrases"][
                        :10
                    ]:  # Only use the first 10 rephrasings
...
class Hallucination(Dataset):
    def __init__(self, split):
        self.data = pd.DataFrame(
            load_dataset("potsawee/wiki_bio_gpt3_hallucination")["evaluation"]
        )
        ...
        if split == "edit":
            ...
            upstream = WebText10k()
            self.text = (
                accurates + originals + upstream.text[:200]
            )  # Add 200 samples from webtext to make sure GPT2 stays good on its training data (200 seems ad hoc but it's actually pretty robust to this choice)
            self.text = [
                {
                    "text": x["text"],
    ...
    def generate_concepts(self):
        ...
        concepts = [
            wikibio["test"]["input_text"][i]["context"]
            .strip()
            .replace("-lrb- ", "")
            .replace(" -rrb-", "")
            for i in bio_idx
    ...
    def get_edits(self, data, concepts):
        ...
        for i in range(len(self.data)):
            ...
            correct_sentences = nltk.sent_tokenize(self.data["wiki_bio_text"][i])[
                : len(annotations)
            ]
