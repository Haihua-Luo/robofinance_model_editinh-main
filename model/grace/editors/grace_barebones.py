...
class GRACE_barebones(torch.nn.Module):
    def __init__(
        self, model, layer, init_epsilon, learning_rate, device, generation=False
    ):
        """
        ...
        setattr(
            edit_module,
            layer_name,
            GRACEAdaptor(original_layer, init_epsilon, device, transpose).to(device),
        )
    ...
    def __call__(self, **kwargs):
        ...
        if self.generation:
            ...
            setattr(
                eval(f"self.model.{self.layer}"), "key_id", key_id
            )  # Tell GRACE which token to use for its query (default is the last token)
...
class GRACEAdaptor(torch.nn.Module):
    ...
    def add_key(self, new_key, new_value):
        ...
        key_labels = self.key_labels + [
            self.edit_label
        ]  # Add new key_label to list of key_labels
    ...
    def forward(self, *args):
        ...
        if (not self.training) & ("keys" not in self.__dict__):
            ...
            if "keys" not in self.__dict__:
                ...
                self.keys, self.values, self.epsilons, self.key_labels = (
                ...
                dists = torch.stack(
                    [euclidean_distance(query, key).view(-1, 1) for key in self.keys]
                ).view(-1, len(query))
